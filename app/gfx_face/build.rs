...
fn main() {
    ...
    if target.contains("android") { android_main() };
}
...
fn android_main() {
    ...
    let toolchain_path = ndk_path.join("toolchains").join(format!("{}-4.9", toolchain)).join("prebuilt").
        join(google_host);
    println!("toolchain path is: {}", toolchain_path.to_str().expect("dogo"));
    ...
    let man_dir = env::var("CARGO_MANIFEST_DIR").ok().unwrap();
    let mut local_lib = PathBuf::from(man_dir);
    local_lib.push("lib");
    local_lib.push(&toolchain);
    ...
    if Command::new(toolchain_path.join("bin").join(format!("{}-gcc", toolchain)))
        ...
        .arg("-I").arg(ndk_path.join("sysroot").join("usr").join("include").join(toolchain.clone()))
        .arg("-I").arg(ndk_path.join("sysroot").join("usr").join("include"))
        .arg("-fPIC")
        .stdout(Stdio::inherit())
        .stderr(Stdio::inherit())
        .status().expect("yolo").code().expect("polio") != 0
    {
    ...
    if Command::new(toolchain_path.join("bin").join(format!("{}-ar", toolchain)))
        ...
        .stdout(Stdio::inherit())
        .stderr(Stdio::inherit())
        .status().expect("ouo").code().expect("owo") != 0
    {
    ...
    //println!("cargo:rustc-link-search=native={}", local_lib.to_string_lossy());
